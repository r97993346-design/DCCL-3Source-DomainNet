...
def main():
    parser = argparse.ArgumentParser(
        description="Run DomainNet fixed-target 10 source-combos, DCCL+ERM"
    )
    parser.add_argument(
        "--target", type=int, required=True, help="Target env id in [0,5]"
    )
    parser.add_argument("--gpu", type=int, default=0, help="GPU id for all jobs")
    parser.add_argument(
        "--data_dir", type=str, required=True, help="DomainNet data root"
    )
    parser.add_argument("--exp_prefix", type=str, default="exp-domainnet-target")
    parser.add_argument(
        "--erm_baseline",
        choices=["weak", "matched"],
        default="weak",
        help=(
            "ERM command mode: weak adds --weak_erm "
            "(ImageNet pretrained, no SWAD); matched keeps ERM hparams "
            "matched to the main run."
        ),
    )
    ...
    for s1, s2, s3 in combos:
        ...
        run(
            base[:2]
            + [f"{args.exp_prefix}_{combo_tag}_erm", "--algorithm", "ERM"]
            + base[2:]
            + erm_extra,
            args.gpu,
        )
