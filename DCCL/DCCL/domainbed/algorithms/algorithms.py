...
class DCCL(Algorithm):
    ...
    def _init_causal_reliability_modules(self, hparams, default_visual_dim):
        ...
        causal_encoder_name = causal_obj.get("encoder_name", "") if isinstance(causal_obj, dict) else ""
        spurious_encoder_name = spurious_obj.get("encoder_name", "") if isinstance(spurious_obj, dict) else ""
        if causal_encoder_name and spurious_encoder_name and causal_encoder_name != spurious_encoder_name:
            raise ValueError(
                "Causal/spurious encoder_name mismatch: "
                f"causal=`{causal_encoder_name}`, spurious=`{spurious_encoder_name}`"
            )
        ...
        if not torch.isfinite(causal_emb).all():
            raise ValueError("Non-finite values found in causal embeddings.")
        if not torch.isfinite(spurious_emb).all():
            raise ValueError("Non-finite values found in spurious embeddings.")
        if isinstance(causal_obj, dict) and isinstance(spurious_obj, dict):
            c_names = causal_obj.get("class_names", None)
            s_names = spurious_obj.get("class_names", None)
            if c_names is not None and s_names is not None and c_names != s_names:
                raise ValueError("Causal/spurious class_names order mismatch.")
            if c_names is not None and len(c_names) != causal_emb.shape[0]:
                raise ValueError(
                    f"Causal class_names length mismatch: len(class_names)={len(c_names)} vs embeddings={causal_emb.shape[0]}"
                )
            if s_names is not None and len(s_names) != spurious_emb.shape[0]:
                raise ValueError(
                    f"Spurious class_names length mismatch: len(class_names)={len(s_names)} vs embeddings={spurious_emb.shape[0]}"
                )
        ...
        metadata_encoder_name = causal_encoder_name or spurious_encoder_name
        if not encoder_name:
            encoder_name = metadata_encoder_name
        if not encoder_name:
            raise ValueError("CSR requires `csr_clip_model` or `encoder_name` metadata in embedding files.")
        if metadata_encoder_name and encoder_name != metadata_encoder_name:
            raise ValueError(
                f"CSR clip model mismatch: requested csr_clip_model=`{encoder_name}` but embedding metadata encoder_name=`{metadata_encoder_name}`"
            )
