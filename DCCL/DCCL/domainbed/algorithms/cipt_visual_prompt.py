"""Shallow learnable visual prompt tokens for a frozen OpenAI CLIP ViT.

Only the prompt tokens are trainable. CLIP visual-backbone parameters stay
frozen, but autograd is intentionally kept through the frozen transformer so
existing downstream CIPT losses can update the visual prompt tokens.
"""
...
class VisualPromptLearner(nn.Module):
    """Shared shallow visual tokens inserted after CLIP's CLS token."""
...
def encode_image_with_visual_prompt(clip_model, images, prompt_learner):
    """Encode images with shallow visual prompting and frozen CLIP weights."""
    visual = clip_model.visual
    ...
    # Do NOT wrap this frozen transformer in torch.no_grad(): gradients must
    # pass through it to the learnable prompt tokens, while CLIP parameters
    # themselves remain requires_grad=False.
    x = x.permute(1, 0, 2)
