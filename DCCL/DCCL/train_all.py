...
def main():
    ...
    parser.add_argument("--use_causal_variant_factor", action="store_true", help="Enable online causal variant factor DCCL")
    parser.add_argument("--causal_use_photometric", type=lambda v: str(v).lower() == "true", default=True)
    parser.add_argument("--causal_use_xdomainmix", type=lambda v: str(v).lower() == "true", default=True)
    parser.add_argument("--causal_use_diffusion", type=lambda v: str(v).lower() == "true", default=True)
    parser.add_argument("--causal_photo_ops", type=str, default="brightness,contrast,color,sharpness,gaussian_noise,blur")
    parser.add_argument("--causal_photo_strength_min", type=float, default=0.1)
    parser.add_argument("--causal_photo_strength_max", type=float, default=0.5)
    parser.add_argument("--causal_xdomainmix_alpha", type=float, default=0.5)
    parser.add_argument("--causal_xdomainmix_same_class_only", type=lambda v: str(v).lower() == "true", default=True)
    parser.add_argument("--causal_xdomainmix_require_diff_domain", type=lambda v: str(v).lower() == "true", default=True)
    parser.add_argument("--causal_xdomainmix_fallback_skip", type=lambda v: str(v).lower() == "true", default=True)
    parser.add_argument("--causal_diffusion_model_path", type=str, default="")
    parser.add_argument("--causal_diffusion_local_only", type=lambda v: str(v).lower() == "true", default=True)
    parser.add_argument("--causal_diffusion_every_n_steps", type=int, default=1)
    parser.add_argument("--causal_diffusion_max_images_per_step", type=int, default=4)
    parser.add_argument("--causal_diffusion_steps", type=int, default=50)
    parser.add_argument("--causal_diffusion_cfg_text", type=float, default=8.5)
    parser.add_argument("--causal_diffusion_cfg_image", type=float, default=1.1)
    parser.add_argument("--causal_diffusion_seed", type=int, default=0)
    parser.add_argument("--causal_diffusion_device", type=str, default="cuda")
    parser.add_argument("--causal_style_per_image", type=int, default=1)
    parser.add_argument("--causal_prompt_mode", type=str, default="target_agnostic")
    parser.add_argument("--causal_prompt_bank", type=str, default="default_dg_style")
    parser.add_argument("--causal_use_pre_anchor_filter", type=lambda v: str(v).lower() == "true", default=True)
    parser.add_argument("--causal_pre_anchor_thresh", type=float, default=0.35)
    parser.add_argument("--causal_cls_filter_mode", type=str, default="confidence", choices=["confidence", "argmax", "none"])
    parser.add_argument("--causal_cls_conf_thresh", type=float, default=0.5)
    parser.add_argument("--causal_filter_warmup_steps", type=int, default=500)
    parser.add_argument("--causal_cls_filter_use_ema", type=lambda v: str(v).lower() == "true", default=False)
    parser.add_argument("--causal_sensitivity_metric", type=str, default="kl", choices=["kl", "l1", "js"])
    parser.add_argument("--causal_sensitivity_temperature", type=float, default=1.0)
    parser.add_argument("--causal_top_m", type=int, default=1)
    parser.add_argument("--causal_sem_weight", type=float, default=0.1)
    parser.add_argument("--causal_kl_weight", type=float, default=0.0)
    parser.add_argument("--causal_kl_warmup_steps", type=int, default=1000)
    parser.add_argument("--causal_cf_as_anchor", type=lambda v: str(v).lower() == "true", default=False)
    parser.add_argument("--causal_save_diffusion_images", type=lambda v: str(v).lower() == "true", default=True)
    parser.add_argument("--causal_save_diffusion_mode", type=str, default="kept", choices=["none", "all", "kept", "selected"])
