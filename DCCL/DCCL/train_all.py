...
def main():
    ...
    # CIRL options. These branches are training-only and do not affect predict()/evaluation.
    parser.add_argument("--use_fourier_intervention", action="store_true", help="Enable cross-domain Fourier amplitude intervention branch")
    parser.add_argument("--use_intervention_reliability", action="store_true", help="Use reliability-weighted SupCon instead of the original SupCon")
    parser.add_argument("--fourier_mix_alpha", type=float, default=1.0, help="Fourier amplitude donor mix ratio; set negative to sample from min/max")
    parser.add_argument("--fourier_mix_min", type=float, default=0.0, help="Minimum Fourier amplitude mix ratio")
    parser.add_argument("--fourier_mix_max", type=float, default=1.0, help="Maximum Fourier amplitude mix ratio")
    parser.add_argument("--intervention_mu", type=float, default=0.5, help="Reliability sigmoid center for z_orig/z_inter similarity")
    parser.add_argument("--intervention_temperature", type=float, default=0.1, help="Reliability sigmoid temperature")
    parser.add_argument("--reliability_min_weight", type=float, default=0.1, help="Minimum positive-pair reliability weight")
    parser.add_argument("--reliability_loss_weight", type=float, default=1.0, help="Weight multiplier for reliability-weighted SupCon")
    parser.add_argument("--use_factorization_loss", action="store_true", help="Enable z_orig/z_inter factorization loss")
    parser.add_argument("--factorization_loss_weight", type=float, default=0.1, help="Weight for CIRL factorization loss")
    parser.add_argument("--factorization_offdiag_weight", type=float, default=0.005, help="Off-diagonal weight for factorization loss")
    parser.add_argument("--factorization_eps", type=float, default=1e-4, help="Numerical epsilon for batch-wise standardization")
    parser.add_argument("--factorization_feature_source", type=str, default="projection", help="Feature source name for factorization bookkeeping")
    parser.add_argument("--use_adversarial_masker", action="store_true", help="Enable CIRL adversarial Gumbel masker")
    parser.add_argument("--masker_aux_loss_weight", type=float, default=0.1, help="Student loss weight for masker auxiliary CE")
    parser.add_argument("--masker_adversarial_weight", type=float, default=1.0, help="Adversarial weight for masker inferior CE")
    parser.add_argument("--mask_keep_ratio", type=float, default=0.5, help="Fraction of projected features kept by Gumbel top-k mask")
    parser.add_argument("--gumbel_temperature", type=float, default=1.0, help="Gumbel top-k temperature")
    parser.add_argument("--gumbel_hard", type=lambda x: str(x).lower() in ("1", "true", "yes", "y"), default=True, help="Use straight-through hard Gumbel top-k masks")
    parser.add_argument("--masker_hidden_dim", type=int, default=512, help="Hidden dimension of the CIRL masker MLP")
    parser.add_argument("--masker_update_interval", type=int, default=1, help="Run one standalone masker update every N student steps")
