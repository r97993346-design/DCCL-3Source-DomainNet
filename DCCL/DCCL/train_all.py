...
def main():
    ...
    parser.add_argument("--use_fourier_intervention", nargs="?", const=True, type=lambda x: str(x).lower() in ["1", "true", "yes"], default=False)
    ...
    parser.add_argument("--fourier_donor_cross_domain_only", nargs="?", const=True, type=lambda x: str(x).lower() in ["1", "true", "yes"], default=True)
    ...
    parser.add_argument("--detach_reliability_score", nargs="?", const=True, type=lambda x: str(x).lower() in ["1", "true", "yes"], default=True)
    parser.add_argument("--log_intervention_stats", nargs="?", const=True, type=lambda x: str(x).lower() in ["1", "true", "yes"], default=True)
    parser.add_argument("--use_factorization_loss", nargs="?", const=True, type=lambda x: str(x).lower() in ["1", "true", "yes"], default=False)
    ...
    parser.add_argument("--factorization_feature_source", type=str, default="contrastive")
    parser.add_argument("--log_factorization_stats", nargs="?", const=True, type=lambda x: str(x).lower() in ["1", "true", "yes"], default=True)
    parser.add_argument("--use_adversarial_masker", nargs="?", const=True, type=lambda x: str(x).lower() in ["1", "true", "yes"], default=False)
    parser.add_argument("--masker_aux_loss_weight", type=float, default=0.1)
    parser.add_argument("--masker_adversarial_weight", type=float, default=1.0)
    parser.add_argument("--mask_keep_ratio", type=float, default=0.5)
    parser.add_argument("--gumbel_temperature", type=float, default=1.0)
    parser.add_argument("--gumbel_hard", nargs="?", const=True, type=lambda x: str(x).lower() in ["1", "true", "yes"], default=True)
    parser.add_argument("--masker_hidden_dim", type=int, default=256)
    parser.add_argument("--masker_update_interval", type=int, default=1)
    parser.add_argument("--log_masker_stats", nargs="?", const=True, type=lambda x: str(x).lower() in ["1", "true", "yes"], default=True)
    parser.add_argument("--use_rise", action="store_true", help="Enable RISE-guided CLIP semantic supervision.")
    parser.add_argument("--use_rise_kd", action="store_true", help="Enable RISE CLIP-KD loss.")
    parser.add_argument("--use_rise_proto", action="store_true", help="Enable RISE CLIP text prototype alignment loss.")
    parser.add_argument("--rise_clip_model_name", type=str, default="ViT-B/32")
    parser.add_argument("--rise_clip_download_root", type=str, default=None)
    ...
    parser.add_argument("--rise_prompt_mode", type=str, default="rise80", choices=["simple", "multi", "domain_invariant", "rise80"])
    parser.add_argument("--rise_freeze_clip", nargs="?", const=True, type=lambda x: str(x).lower() in ["1", "true", "yes"], default=True)
